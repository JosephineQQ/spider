...
//		Filter
	...
	for (Result result : scanner) {
		if(result.raw().length==0){
			System.out.println(tableName+" table is empty！");
		}else{
	...
	private void deleteTable(String tableName) {
		try {
			if (admin.tableExists(tableName)) {
				...
				System.out.println(tableName+"successfully deleted！");
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void deleteOneRecord(String tableName, String rowKey) {
		...
		try {
			table.delete(delete);
			System.out.println(rowKey+"record deleted sucessfully！");
		} catch (IOException e) {
			e.printStackTrace();
			System.out.println(rowKey+"record delete failed！");
		}
	}
	/**
	 * 获取表的所有数据
	...
	public void createTable(String tableName, String column) throws Exception {
		if(admin.tableExists(tableName)){
			...
			System.out.println(tableName+"create table successfully！");
		}
	}
}
